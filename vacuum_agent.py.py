...
class Agent:
    ...
    def think(self,x,y):
        if self.dirty:
            ...
            choice=random.choice([self.ActionType.UP, self.ActionType.DOWN, self.ActionType.LEFT, self.ActionType.RIGHT])
            #########################
            if choice==self.ActionType.UP and check[y][x][0]==False:
                choice=random.choice([ self.ActionType.DOWN, self.ActionType.LEFT, self.ActionType.RIGHT])##########
                if choice==self.ActionType.DOWN and check[y][x][1]==False :
                    choice=random.choice([  self.ActionType.LEFT, self.ActionType.RIGHT])
                    if choice==self.ActionType.LEFT and check[y][x][2]==False :
                        return self.ActionType.RIGHT
                    else:
                        return self.ActionType.LEFT
                
                    
                elif choice==self.ActionType.LEFT and check[y][x][2]==False :
                    choice=random.choice([  self.ActionType.DOWN, self.ActionType.RIGHT])
                    if choice==self.ActionType.DOWN and check[y][x][1]==False :
                        return self.ActionType.RIGHT
                    else:
                        return self.ActionType.DOWN
                
                elif choice==self.ActionType.RIGHT and check[y][x][3]==False :
                    choice=random.choice([  self.ActionType.DOWN, self.ActionType.LEFT])
                    if choice==self.ActionType.DOWN and check[y][x][1]==False :
                        return self.ActionType.LEFT
                    else:
                        return self.ActionType.DOWN
            ...
            #################################
            elif choice==self.ActionType.DOWN and check[y][x][1]==False:
                choice=random.choice([ self.ActionType.UP, self.ActionType.LEFT, self.ActionType.RIGHT])##########
                if choice==self.ActionType.UP and check[y][x][0]==False :
                    choice=random.choice([  self.ActionType.LEFT, self.ActionType.RIGHT])
                    if choice==self.ActionType.LEFT and check[y][x][2]==False :
                        return self.ActionType.RIGHT
                    ...
                    choice=random.choice([  self.ActionType.UP, self.ActionType.RIGHT])
                    if choice==self.ActionType.UP and check[y][x][0]==False :
                        return self.ActionType.RIGHT
                    else:
                        return self.ActionType.UP
                
                elif choice==self.ActionType.RIGHT and check[y][x][3]==False :
                    choice=random.choice([  self.ActionType.UP, self.ActionType.LEFT])
                    if choice==self.ActionType.UP and check[y][x][0]==False :
                        return self.ActionType.LEFT
                    else:
                        return self.ActionType.UP
            ...
            ###################################
                ...
                choice=random.choice([ self.ActionType.UP, self.ActionType.DOWN, self.ActionType.RIGHT])##########
                if choice==self.ActionType.UP and check[y][x][0]==False :
                    choice=random.choice([  self.ActionType.DOWN, self.ActionType.RIGHT])
                    if choice==self.ActionType.DOWN and check[y][x][1]==False :
                        return self.ActionType.RIGHT
                    ...
                    choice=random.choice([  self.ActionType.DOWN, self.ActionType.UP])
                    if choice==self.ActionType.DOWN and check[y][x][1]==False :
                        return self.ActionType.UP
                    else:
                        return self.ActionType.DOWN
            ...
            ###################################
                ...
                choice=random.choice([ self.ActionType.UP, self.ActionType.DOWN, self.ActionType.LEFT])##########
                if choice==self.ActionType.UP and check[y][x][0]==False :
                    choice=random.choice([  self.ActionType.DOWN, self.ActionType.LEFT])
                    if choice==self.ActionType.DOWN and check[y][x][1]==False :
                        return self.ActionType.LEFT
...
# random.random(): Returns a random float number between 0 and 1
    ...
    def accept_action(self, action):
        ...
        if action == Agent.ActionType.SUCK:
            ...
            if self.maze[self.agent_y-1][self.agent_x] != self.OBSTACLE:
                self.agent_y -= 1
            else:
                self.bump = True
                check[self.agent_y][self.agent_x][0]=False
        elif action == Agent.ActionType.DOWN:
            if self.maze[self.agent_y+1][self.agent_x] != self.OBSTACLE:
                self.agent_y += 1
            else:
                self.bump = True
                check[self.agent_y][self.agent_x][1]=False
        elif action == Agent.ActionType.LEFT:
            if self.maze[self.agent_y][self.agent_x-1] != self.OBSTACLE:
                self.agent_x -= 1
            else:
                self.bump = True
                check[self.agent_y][self.agent_x][2]=False
            ...
            if self.maze[self.agent_y][self.agent_x+1] != self.OBSTACLE:
                ...
                self.bump = True
                check[self.agent_y][self.agent_x][3]=False
    ...
    def draw_map(self, scene):
        for i, row in enumerate(self.maze):
            for j, cell in enumerate(row):
                ...
                color = QColor.fromRgb(dirty_color, dirty_color, dirty_color) if cell != self.OBSTACLE else QColor(Qt.blue)
                scene.addRect(j * self.CELL_SIZE, i * self.CELL_SIZE, self.CELL_SIZE, self.CELL_SIZE, QPen(color), color)
...
class MainWindow(QMainWindow):
    ...
    def do_one_step(self):
        if self.current_time < self.agent.life_time:
            ...
            #env1=Environment
            self.action = self.agent.think(self.env.get_x(),self.env.get_y())
